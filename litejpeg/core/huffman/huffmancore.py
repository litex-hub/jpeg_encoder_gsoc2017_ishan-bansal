...
The module takes an input in the form of matrix containing 64 blocks with 20 bits
each and give an output of variable number each having a bit_length of 8 bits.
...
class HuffmanDatapath(Module):
    ...
    def __init__(self):
        ...
        width_word = 16+7
        # Stores data for giving the output when the bit_ptr reaches more than 8 bits.
        ...
        # vlc_dc, vlc_dc_size : Get the data and number of bits out to the DC ROM.
        # vlc_ac, vlc_ac_size : Get the data and number of bits out to the AC ROM.
        vlc_dc = Signal(16)
        vlc_dc_size = Signal(5)
        vlc_ac = Signal(16)
        vlc_ac_size = Signal(5)
        # Decide wheather the data_out is DC or AC depending on the first_rle_word.
        vlc_d = Array(Signal() for i in range(16))
        vlc_size_d = Signal(5)

        # Get the output from the Amplitude and Number of bits required to store
        # the amplitude.
        ...
        # For extracting the information for the Amplitude and size of the amplitude.
        self.comb += [
        [If( j < vli_ext_size,
        ...
        # VLC state
           ...
           If(ready_one,
                ...
                [If( i+(num_fifo_wrs*8) < width_word,
                       word_reg[width_word-1-i].eq(word_reg[width_word-1-(num_fifo_wrs*8)-i]))
                         for i in range(width_word)],
...
class HuffmanEncoder(PipelinedActor, Module):
    ...
    The input is been taken from the 'sink' with the Endstream of length 20 bits
    and the output is been sent by using 'source' with 9 bits.
