...
As the input given to this module is in the form of serial input of the value
of approximately 64*12 bits. This will go into the DCTdatapath where
DCT1d conversion should take place on the matrix formed after making 64 values
of 12 bits each out of the serial input. Then the transpose of the resltant
matrix is obtained and dct1D is again applied on this matrix. Finally the
matrix which is been obtained is been converter into the serial output
of 64*12 bits.
'''
datapath_latency = 8
...
class DCTDatapath(Module):
    ...
    -----------------------------------------
    This will take the input as YCbCr matrix and than convert
    it into the DCT matrix. The matrix so formed is been passed
    through the zigzag or the quantization module so that the
    compression done will be maximum.

    Input:
    ------
    It takes the input in the form of 64 blocks of 8 bits
    each as unsigned int.
    ...
    def __init__(self, dw, dct_block):
        ...
        Taking its values from the dct_dalayed passed as an
        input to the DCT1d module.
        ...
        Transpose vector_out and take the values to pass as an input
        to the DCT1d module.
        ...
        dct_matrix_1d = Array(Array(Signal(dw) for a in range(8))
                              for b in range(8))

        # intialise the vectors for 8 blocks at a time
        # that will undergo DCT processing
        # at a time.
        ...
        # Since the output doesn't come in a single clock cycle.
        # Hence there is a need of providing delay in the output which
        # is determined by datapath_latency of the module and provided
        # as below:
        dct_delayed = [sink]
        for i in range(datapath_latency):
            ...
            for k in range(dct_block):
                name = "dct_" + str(k)
                self.sync += getattr(dct_n, name).eq(
                                                getattr(dct_delayed[-1], name))
            dct_delayed.append(dct_n)

        # Assigning vectors 8 blocks for which the DCT conversion
        # has to take place.
        for j in range(8):
            for y in range(8):
                name = "dct_"+str((8*y)+j)
                self.sync += vector[y][j].eq(
                                            getattr(dct_delayed[-1], name)-128)

        # Assign DCT to every block to get the result.
        for w in range(8):
            self.dct1D(vector[w], dw, vector_out[8*w:(w+1)*8])

        # Now intialising the vectors for applying the DCT function
        # on the transpose matrix.
        ...
        for r in range(64):
            ...
            for t in range(3):
                self.sync += vector2final_outdivide[r][9+t].eq(
                    vector2final_outdivide[r][8])
    ...
    def dct1D(self, vector1d, dw, vector1d_out):
        ...
        """
        This will do the DCT operation over a single row or column.
        This will get the DCT2D by providing DCT1d over each of
        the row and than take the transpose and than do the DCT1d
        over each column.
        ...
        """
        This migen doesn't support floating values for signals.
        Hence the following method is used for the purpose.
        ...
        self.Butterfly(
            vector10[7], vector10[4], vector1d_out[1], vector1d_out[7])
...
class DCT(PipelinedActor, Module):
    """
    This will get the input into the DCT module in the
    form of 64 blocks with 8 bits each.
    The input is taken in the form of serial input.
    After obtaining all the 64 inputs from the serial input
    they are than pass to the DCT module.
    Also after obtaining the 64 blocks from the DCT module,
    these are transferred to the output in the form of serial data.
    ...
    def __init__(self, dw=12, dct_block=64):
        ...
        # Setting the datapath which takes the YCbCr input
        # and give DCT output both
        # in the form of 64 blocks.
        self.submodules.datapath = DCTDatapath(dw, dct_block)
        self.comb += self.datapath.ce.eq(self.pipe_ce)

        """
        Stores the data obtained from the serial input into the
        memory and than put into the DCT module. These data
        than after processing will again obtained
